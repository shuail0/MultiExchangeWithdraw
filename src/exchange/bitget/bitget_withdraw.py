...
def bitget_withdraw(exchange, wallet_address, tag=None, currency=None, amount=None, chain=None):
    """
   在 bitget 交易所上执行提币操作。
   ...
   """
    ...
    # 检查资金账户余额
    balance = pd.DataFrame(exchange.fetch_balance()['info'])  # 获取资金账户余额
    balance.set_index('coinName', inplace=True, drop=True)
    balance = balance.to_dict('index')
    # 获取可用余额
    free_balance = float(balance[currency]['available'])
    print(f"可用 {currency} 余额：{free_balance}")

    if free_balance >= (amount):
        # 提现
        print(f"正在将 {amount} {currency} 提现到钱包地址 {wallet_address} 提币链 {chain}")
        params = {'coin': currency, 'chain': chain, 'address': wallet_address, 'amount': amount}
        if tag is not None:
            params['tag'] = tag
        withdrawal = exchange.privateSpotPostWalletWithdrawalV2(params)
